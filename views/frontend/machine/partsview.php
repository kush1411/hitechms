            ...
            <div class="row">
                <div class="col-lg-12 margin-bottom-10">
                    ...
                    if ($userRights['add'] && $machine->MachStatus == 0) {
                        ?>
                        <a href = "<?php echo base_url() ?>machine/addparts/<?=$machine->MachID?>" class="btn btn-info btn-xs btn-raised pull-right">Add Machine Parts</a>
                    <?php } ?>
                    <?php
                ...
                <?php
                    if ($userRights['delete']) {
                        $frmaction = 'machine/deleteparts/'.$machine->MachID;
                                            ...
                                            <td class="center">
                                                <?php
                                                    if ($userRights['edit'] && $machine->MachStatus == 0) {
                                                        ?>
                                                        <a href="<?php echo base_url() ?>machine/editparts/<?=$machine->MachID?>/<?php echo $user->CmpMachPartID; ?>" title="Edit" class="btn-edit"><i class="md md-edit btn btn-success btn-xs btn-raised "></i></a>
                                                    <?php } if ($userRights['delete'] && $machine->MachStatus == 0) { ?>
                                                        <a href="<?php echo base_url() ?>machine/deleteparts/<?=$machine->MachID?>/<?php echo $user->CmpMachPartID ?>"  title="Delete" class="btn-cross alertme"><i class="md md-delete btn btn-danger btn-xs btn-raised "></i></a>        
                                                            <?php
                                                        }
                                                        ?>
                                            </td>
