 ...
 * 
 */
class My_operator extends MY_base {

    function __construct() {
        parent::__construct();
        $this->front_js_css();
        $this->data['folder_type'] = OPERATORFOLDERNAME;
        if(!$this->session->userdata(SITE_NAME . '_operator_data')){ 
            redirect('/operator');
        }else{
//            $seesiondata = $this->session->userdata(SITE_NAME . '_operator_data');
//            $CompID = $seesiondata['CompID'];
//            $this->data['CompDetails'] = Userprofile::find('first',array('conditions' => array('CompID = ?', $CompID)));
        }
    }
    ...
    function _api_require_user($params = array()) {
        ...
        if (count($params['only']) > 0 AND !in_array($this->data['action'], $params['only']))
            return TRUE;
        if ($this->session->userdata(SITE_NAME . '_operator_data') && $this->session->userdata[SITE_NAME . '_operator_data']['CompID'] > 0) {
    ...
    function _get_auto_id() {
        ...
        $refCompID = $this->session->userdata(SITE_NAME . '_operator_data')['CompID'];
