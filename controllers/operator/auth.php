...
Class auth extends MY_base {
    ...
    function __construct() {
        ...
        if($this->session->userdata(SITE_NAME . '_operator_data') && $this->data['action'] == 'index'){ 
    ...
    function index() {
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run()) {
                try {
                    $code = $this->operatormst->checkCompCode($this->form_validation->set_value('code'));
                    if(!$code){
                        $this->_show_message("Company Code Not Registered", "error");
                        redirect('/operator');
                        die;
                    }
                    $operator = $this->operatormst->getOperatorByID($this->form_validation->set_value('email'),$this->form_validation->set_value('code'));

                    if ($operator && $this->validate_password($operator, $this->form_validation->set_value('password'))) {
                        if ($operator->EmpStatus == 0) {
                            $this->_show_message("Account is not activated, Contact Your Administrator", "error");
                            redirect('/operator');
                        }
                        $this->session->set_userdata(SITE_NAME . '_operator_data', array(
                            'operator_id' => $operator->EmpID,
                            'CompID' => $operator->refCompID,
                            'status' => $operator->EmpStatus,
                            'firstname' => $operator->EmpName,
                            'IsQuot' => $operator->IsQuot
                        ));
                        redirect('/operator/dashboard');
                    } else {
    ...
    public function logout() {
        $this->session->unset_userdata(SITE_NAME . '_operator_data');
        ...
        if ($this->tank_auth->is_logged_in()) {         // logged in
            ...
            if ($this->form_validation->run()) {
                if (!is_null($data = $this->tank_auth->forgot_password(
                    ...
                    redirect('operator/auth/forgotpass');
                }
            } else {
                $this->data['postdata'] = $this->input->post();
    ...
    public function change_password() {
        ...
        if ($this->tank_auth->is_logged_in()) {        // not logged in or not activated
            ...
            if ($this->input->post()) {
                ...
                $this->form_validation->set_rules('new_password', 'New Password', 'trim|required|xss_clean|min_length[' . $this->config->item('password_min_length', 'tank_auth') . ']|max_length[' . $this->config->item('password_max_length', 'tank_auth') . ']|alpha_dash');
                $this->form_validation->set_rules('confirm_new_password', 'Confirm new Password', 'trim|required|xss_clean|matches[new_password]');
                $user_id = $this->session->userdata[SITE_NAME . '_operator_data']['user_id'];
                $data['errors'] = array();
                if ($this->form_validation->run()) {
